...
![Instalar la versión por defecto](postgres/postgres-install-default-ubuntu.webp)

Como puedes notar, se nos instalará PostgreSQL, pero no la versión más reciente. Para instalar una versión específica, puedes __añadir el repositorio oficial de PostgreSQL__ de la siguiente manera:
...
![Agregar el repositorio](postgres/postgres-add-repository.webp)

Ahora agrega el repositorio de PostgreSQL a la lista de fuentes de software:

```terminal
echo "deb http://apt.postgresql.org/pub/repos/apt/ $(lsb_release -cs)-pgdg main" | sudo tee /etc/apt/sources.list.d/pgdg.list
```

Con esto, volvemos a actualizar con `sudo apt update` y luego ya puedes instalar PostgreSQL con una versión más reciente:
